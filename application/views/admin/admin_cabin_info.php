<div class="">
	<?php
	// var_dump($cabin_info);
	foreach ($cabin_info as $cabindata): ?>

		<!-- Back Button -->
		<div class="flex justify-between items-center mb-6">
			<h1 class="text-3xl font-bold text-gray-800">Cabin Information</h1>
		...
		<div class="bg-white shadow-lg rounded-lg p-6 mb-8">
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				<!-- Vessel Name -->
				<div>
					<label for="cabin_name" class="block text-sm font-medium text-gray-700 mb-1">Cabin Name</label>
					<input type="text" id="cabin_name" name="cabin_name" class="border-gray-300 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm w-full"
						value="<?php echo htmlspecialchars($cabindata['cabin_name']); ?>" onchange="enableSaveButton()">
				</div>

				<!-- Vessel Thumbnail -->
				<div>
					<label for="vessel_thumbnail" class="block text-sm font-medium text-gray-700 mb-1">Cabin Thumbnail</label>
				...
				<!-- Vessel Name -->
				<div>
					<label for="cabin_price" class="block text-sm font-medium text-gray-700 mb-1">Cabin Price</label>
					<input type="text" id="cabin_price" name="cabin_price" class="border-gray-300 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm w-full"
						value="<?php echo htmlspecialchars($cabindata['cabin_price']); ?>" onchange="enableSaveButton()">
				</div>
				<!-- Vessel Name -->
				<div>
					<label for="surcharge_percentage" class="block text-sm font-medium text-gray-700 mb-1">Cabin Surcharge(%)</label>
					<input type="text" id="surcharge_percentage" name="surcharge_percentage" class="border-gray-300 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm w-full"
						value="<?php echo htmlspecialchars($cabindata['surcharge_percentage']); ?>" onchange="enableSaveButton()">
				</div>
			...
			<div class="flex justify-end mt-6">
				<button id="save-button" class="bg-green-500 text-white hover:bg-green-600 transition rounded px-4 py-2 shadow-md"
					style="display:none;" onclick="savecabindata(<?php echo htmlspecialchars($cabindata['id']); ?>)">Save Changes</button>
			</div>
		</div>

		<!-- Add Photo Button and Photos Section -->
		<div class="flex justify-between items-center mb-6">
			<h2 class="text-2xl font-semibold text-gray-800">Cabin Photos</h2>
		...
		<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
			<?php
			$photos = explode(';', $cabindata['cabin_photos']);
			...
			?>
				<div class="relative bg-white shadow-md rounded-md overflow-hidden">
					...
					<button class="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 shadow hover:bg-red-600 transition"
						onclick="deletePhoto('<?php echo htmlspecialchars($photo); ?>',<?php echo $cabin_info[0]['id'] ?>)">
	...
	<div id="photo-modal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center hidden">
		<div class="bg-white rounded-lg shadow-lg w-96 p-6 relative">
			...
			<button class="bg-blue-500 text-white hover:bg-blue-600 transition rounded px-4 py-2 mt-4 w-full shadow-md" onclick="uploadPhoto(<?php echo $cabin_info[0]['id']  ?>)">Upload</button>
		</div>
	</div>
</div>
...
<script>
	function deletePhoto(photoPath, cabin_id) {
		if (confirm('Are you sure you want to delete this photo?')) {
			$.ajax({
				url: '<?php echo base_url("cabin/delete_cabin_photo"); ?>',
				type: 'POST',
				dataType: 'json',
				...
				success: function(data) {
					if (data.success) {
						...
						$.ajax({
							url: '<?= base_url('cabin/admin_cabin_info') ?>', // Adjust the endpoint if necessary
							method: 'POST',
							data: {
								id: cabin_id
							},
...
<script>
	...
	function uploadPhoto(vessel_id) {
		...
		$.ajax({
			...
			success: function(response) {
				...
				$.ajax({
					url: '<?= base_url('vessel/admin_cabin_info') ?>',
					method: 'POST',
					data: {
	...
	function savecabindata(vesselId) {
